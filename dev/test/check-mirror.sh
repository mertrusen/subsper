...
# scripts/sync-to-extension.sh keeps them in step by hand, which means a fix
...
if ! diff -qr assets/fonts extension/assets/fonts >/dev/null 2>&1; then
    echo "OUT OF SYNC: extension/assets/fonts differs from assets/fonts"
    fail=1
fi
