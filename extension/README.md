...
3. In **Subtitles → Settings**, choose the spoken language and the source track if automatic audio selection picks music or another sound. Auto tries video sound first; use A1/A2 for a separate microphone. Toggle the line preview and choose its font, size, weight, tracking, and caption-box width. Only the word estimated to start a third line is highlighted. Automatic splitting of new transcripts is optional and off by default; a separate button splits current captions. Click a word to split, use **↑** or drag a caption number onto a neighbor to merge, and use **Cmd/Ctrl+Z** to undo. The estimate never blocks sending; check the final wrapping in Premiere.
